...
if not os.path.isfile(f'instance/{DB_URI}'):
...
def admin_only(f):
    ...
    def decorated_function(*args, **kwargs):
        if not current_user.is_authenticated:
            return abort(403, description='Admin access is required to perform this function.')
...
def register():
    ...
    if form.validate_on_submit():
        ...
        password = generate_password_hash(form.password.data, method="pbkdf2:sha256", salt_length=6)
